...
my $help              = 0;
my $host              = undef;
my $port              = $ENV{ESCAPE_PORT} || $ENV{CATALYST_PORT} || 3000;
my $keepalive         = 0;
my $restart           = $ENV{ESCAPE_RELOAD} || $ENV{CATALYST_RELOAD} || 0;
my $background        = 0;
my $pidfile           = undef;
...
escape_server.pl - Catalyst Testserver

=head1 SYNOPSIS

escape_server.pl [options]
